  ...
  .then(contactResult => {
    ...
    var contactId = contactResult.rows[0].sfid;
      ...
      .then(teamMemberResult => {
        ...
        var projectTeamQueryText = 'SELECT pr.Name as name, pr.sfid as prjsfid , tm.sfid as team__c ,pr.project__c as project__c FROM salesforce.Team__c tm INNER JOIN  salesforce.Project_Team__c pr on pr.sfid = tm.Project_Team__c WHERE tm.sfid IN (' + projectTeamparams.join(',') + ')';
  ...
  var userId = request.user.sfid; 
        ...
        .then((teamMemberResult) => {
            ...
            if(teamMemberResult.rowCount > 0)
            {
                ...
                {   //SELECT sfid, Name, project__c FROM salesforce.Project_Team__c WHERE Team__c IN (' + lstTeamIdParams.join(',') + ')
                    var projectTeamQueryText = 'SELECT pr.Name as name, pr.sfid as sfid , tm.sfid as team__c ,pr.project__c as project__c FROM salesforce.Team__c tm INNER JOIN  salesforce.Project_Team__c pr on pr.sfid = tm.Project_Team__c WHERE tm.sfid IN (' + lstTeamIdParams.join(',') + ')';
                    pool
                    .query(projectTeamQueryText,lstTeamId)
...
//SELECT id,name,sfid,Project__c,Team__c FROM salesforce.Project_Team__c
  pool.query('SELECT pr.Name as name, pr.sfid as sfid , tm.sfid as team__c ,pr.project__c as project__c FROM salesforce.Team__c tm INNER JOIN  salesforce.Project_Team__c pr on pr.sfid = tm.Project_Team__c')
  .then((projTeamResult)=>{
...
router.get('/getTeamdetails',verify,async(request,response)=>{
  ...
  */
  ...
  if(selproject!='allproject'){ //SELECT id,name,sfid,Project__c,Team__c FROM salesforce.Project_Team__c
    projectTeamQuery='SELECT pr.Name as name, pr.sfid as sfid , tm.sfid as team__c ,pr.project__c as project__c FROM salesforce.Team__c tm INNER JOIN  salesforce.Project_Team__c pr on pr.sfid = tm.Project_Team__c WHERE pr.Project__c=$1';
...
'                   FROM salesforce.Milestone1_Task__c tsk '+
  ...
  /* Start of all project */
    ...
    'INNER JOIN salesforce.Team__c team ON projteam.sfid = team.Project_Team__c  '+ //'INNER JOIN salesforce.Team__c team ON projteam.team__c =  team.sfid '+
  ...
  //console.log('inside fun');
        data.rows.forEach((eachRecord,i) => {
          ...
          //console.log(eachRecord);
            ...
            obj.createdNameId = eachRecord.creatednameid;
            obj.sequence = i;
...
router.get('/getTaskListReporting',verify, async (request,response)=>{
  ...
  let str = `Select id,sfid,Name,Email,Employee_ID__c,reporting_manager__c FROM salesforce.Contact `;
   ...
   let queryText = 'SELECT tsk.Id,tsk.sfid as sfids,usr.Name as userName,cont1.name as createdname,cont1.sfid as creatednameid , usr.sfid as userSfid,tsk.Task_Assigned_by__c as assignedBy,tsk.Task_Type_Category__c as function,tsk.name as tskname,tsk.Task_Stage__c as stage,tsk.start_date__c ,tsk.Project_Name__c,tsk.Total_Hours__c ,tsk.assigned_manager__c,tsk.end_time__c,tsk.Task_Type__c,tsk.Planned_Hours__c,tsk.Start_Time__c,cont.sfid as contid ,cont.name as contname,proj.name as projname,tsk.createddate '+
                   ...
                   'INNER JOIN salesforce.Milestone1_Project__c proj ON tsk.Project_Name__c= proj.sfid '+
                   'INNER join (select sfid,Name from salesforce.Contact ct union all select sfid,Name from salesforce.USER us ) cont1 on tsk.Task_Assigned_by__c = cont1.sfid OR cont1.sfid = tsk.Task_Assigned_by_Salesforce__c '+
                   `WHERE  tsk.Assigned_Manager__c IN ${strings}  AND tsk.sfid IS NOT NULL `;
...
router.get('/getRepList',(request,response)=>{
  ...
  if(current.toLowerCase() == 'reportmanager'){
     query = `Select c.id,c.sfid,c.Name,c.Email,c.Employee_ID__c,c.reporting_manager__c,con.name as reportingname  FROM salesforce.Contact c INNER JOIN  salesforce.contact con on con.sfid = c.reporting_manager__c  `;
  }
  else if(current.toLowerCase() == 'teamview') {


    query =  `Select mb.sfid as memberId,con.Name,c.name as reportingname ,con.Email,con.Employee_ID__c,con.reporting_manager__c,con.sfid as conId FROM salesforce.Team_Member__c mb INNER JOIN salesforce.contact con on con.sfid = mb.Representative__c INNER JOIN salesforce.contact c on c.sfid = con.reporting_manager__c  Where mb.Team__c IN (SELECT sfid FROM salesforce.Team__c WHERE Manager__c = '${userId}')`
