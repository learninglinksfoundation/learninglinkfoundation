         ...
         { 
            ...
            dMsg = `Please select ${zone[i]} District`;
             ...
             if(result.error){
                ...
                console.log('1',nonItFormResult[i]);
...
router.get('/getApprovalHistory/:approvalId&:parentAssetId',verify,(request,response)=>{
    let objUser=request.user;
    console.log('user '+objUser);
    let approvalId = request.params.approvalId;
    let parentAssetId = request.params.parentAssetId;
    console.log('parentAssetId  '+parentAssetId);
     isDisabled =  false;
    console.log(' ++++ isDisabled ++++ '+isDisabled); 
    response.render('approvalHistoryView',{objUser,isDisabled,approvalId,parentAssetId: parentAssetId});
...
router.get('/getProcurementDetail',async(request,response)=>{
    ...
    console.log('getProcurementITDetail Id='+procurementId);
    let procDetail={};
    let qry='SELECT proc.sfid,proc.Name as procName ,proc.Geographic_Zone__c,proc.District_Upper_Zone__c,proc.Districts_Lower_Zone__c,asst.name as astname ,proc.Catgeory__c,proc.Total_Approved_Budget_for_the_purchase__c as purchasebudg  ,proc.Asset_Requisition_Form__c,proc.Items__c ,proc.Others__c,proc.Products_Services_Name__c,vend.name as vendorName,proc.Product_Service__c,proc.Quantity__c, proc.Budget__c,proc.Impaneled_Vendor__c, '+
    ...
    'INNER JOIN salesforce.Impaneled_Vendor__c vend '+
    'ON proc.Impaneled_Vendor__c =  vend.sfid '+
    'INNER JOIN salesforce.Asset_Requisition_Form__c asst '+
    'ON proc.Asset_Requisition_Form__c =  asst.sfid '+
    'WHERE proc.sfid=$1';
    pool
    .query(qry,[procurementId])
    .then((querryResult)=>{
...
router.get('/getVendorDetail',async(request,response)=>{
    ...
    await
    pool
    .query('select imp.sfid ,imp.Contact__c,con.Name as username,imp.geographic_zone__c,imp.District_Lower_Zone__c,imp.Districts_Upper_Zone__c,imp.name,imp.vendor_Name__c ,imp.contact_no__c,imp.name_of_signing_authority__c,imp.bank_details__c,imp.pan_no__c,address__c,imp.GST_No__c,imp.Reason_for_not_providing_GST_no__c,imp.Bank_IFSC_Code__c ,imp.Bank_Account_No__c,imp.ownerid,imp.State__c,imp.District__c '+
    'FROM salesforce.Impaneled_Vendor__c imp INNER join salesforce.contact con on con.sfid = imp.Contact__c  where sfid =$1 ',[vendorId])
    .then((queryResult)=>{
