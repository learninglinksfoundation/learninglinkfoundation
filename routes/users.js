...
router.post('/updatePass',(request,response)=>{
  ...
  const { pass,pass2,user}=request.body;
   ...
   const schema = joi.object({
    ...
    pass:joiPassword .string().required().label('Password must be 10 characters long'),
...
router.get('/editProfile',verify,(request,response)=>{
  ...
  let queryContact = 'SELECT c.sfid, c.profile_picture_url__c, c.email, c.employee_id__c,c.reporting_manager__c ,Salesforce_Reporting_Manager__c, c.pm_email__c, c.employee_category_band__c, c.address__c,c.mobilephone, c.name FROM salesforce.contact c  where c.sfid=$1  ' ;
  pool
