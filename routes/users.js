...
//////////////////////////////////////////////////////////////////////////////////////
  ...
  let isActive=true;
   ...
   if (!email) {
     errors.push({ msg: 'Please enter correct email' });
...
router.post('/sendEMail',(request,response)=>{
 ...
 /*  nodemailer.createTestAccount((err, account) => {
  ...
  let message = {
    from: '"LLF_Heroku_Support"<crm@learninglinksindia.org>',
    to:email,
    subject: 'Finish Resetting your Heroku Password ',
    text: 'Plz Click the below link to generate your password',
    html: 'Heroku recently received a request to reset the password for the username '+email +'. <br/><br/>To finish resetting your password, go to the following link. <br/><br/> This link expires in 24 hours<br/><br/> Link : https://learninglinksfoundation-test.herokuapp.com/users/resetPassword/'+sfid
...
router.post('/updatePass',(request,response)=>{
  ...
  const schema = joi.object({
    ...
    pass:joi.string().regex( /^(?!.*\s)(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])(?=.*[~`!@#$%^&*()--+={}\[\]|\\:;"'<>,.?/_₹]).{10,15}$/).required().label('Password must contain 10 digits 1 number, 1 uppercase, 1 lowercase and 1 specialcharacter'),
 ...
 pool
  ...
  .then( async (queryResult)=>{
   ...
   if(repid && repid.substring(0,3)=='003') {
    ...
    userdetail.reportingnm = heresp.rows.length > 0 ?heresp.rows[0].name : '';
    let salresp = await pool.query('Select sfid,name from salesforce.user where sfid=$1',[userdetail.salesforce_reporting_user__c])
    userdetail.salreportingnm = salresp.rows.length > 0 ?salresp.rows[0].name : '';
